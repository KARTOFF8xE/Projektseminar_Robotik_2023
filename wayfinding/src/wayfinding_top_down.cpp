...
cv::Point2d wayfinding::top_down::getVanishingPoint(const cv::Mat& K, double pan, double tilt) {
    /**
...
bool wayfinding::top_down::getTransformation(cv::Mat& M, std::vector<cv::Point2i>& points, const cv::Size& image_size, const cv::Point2i& vanishing_point, double rel_upper_line_pos) {
    /** Concept:
     * unkown: x
     * known: A, B, y
     * 
...
bool wayfinding::top_down::getTransformation(cv::Mat& M, const cv::Size& image_size, const cv::Point2i& vanishing_point, double rel_upper_line_pos) {
    //move bottom left and bottom right further out of the window to cover a wider window
...
void wayfinding::top_down::transformToTopDown(cv::InputArray src, cv::OutputArray dst, cv::InputArray M, const cv::Size& dst_size) {
    cv::warpPerspective(src, dst, M, dst_size, cv::INTER_NEAREST);
}

cv::Point2i wayfinding::top_down::unwarpPoint(const cv::Mat& transformation_matrix, const cv::Point2i& point) {
